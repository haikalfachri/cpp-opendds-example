...
int ACE_TMAIN(int argc, ACE_TCHAR* argv[]) {
    try {
        ...
        // Register TypeSupport (HelloWorld::Hello)
        HelloWorld::HelloTypeSupport_var ts = new HelloWorld::HelloTypeSupportImpl;
        ...
        DDS::Topic_var topic = participant->create_topic("Movie Discussion List", type_name, TOPIC_QOS_DEFAULT, 0,
        ...
        HelloWorld::HelloDataWriter_var message_writer = HelloWorld::HelloDataWriter::_narrow(writer);
        ...
        while (true) {
            ...
            if (writer->get_publication_matched_status(matches) != ::DDS::RETCODE_OK) {
                ACE_ERROR_RETURN(
                    (LM_ERROR, ACE_TEXT("ERROR: %N:%l: main() -") ACE_TEXT(" get_publication_matched_status failed!\n")), 1);
            }
        ...
        ACE_DEBUG((LM_DEBUG, ACE_TEXT("Subscriber is available\n")));
        ...
        if (message_writer->wait_for_acknowledgments(timeout) != DDS::RETCODE_OK) {
            ACE_ERROR_RETURN((LM_ERROR, ACE_TEXT("ERROR: %N:%l: main() -") ACE_TEXT(" wait_for_acknowledgments failed!\n")),
                             1);
        }
