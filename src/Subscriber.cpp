...
int ACE_TMAIN(int argc, ACE_TCHAR* argv[]) {
    try {
        ...
        // Register Type (HelloWorld::Hello)
        ...
        DDS::DataReader_var reader =
            subscriber->create_datareader(topic, reader_qos, listener, OpenDDS::DCPS::DEFAULT_STATUS_MASK);

        if (!reader) {
            ACE_ERROR_RETURN((LM_ERROR, ACE_TEXT("ERROR: %N:%l: main() -") ACE_TEXT(" create_datareader failed!\n")), 1);
        }

        HelloWorld::HelloDataReader_var reader_i = HelloWorld::HelloDataReader::_narrow(reader);
        ...
        while (true) {
            ...
            if (reader->get_subscription_matched_status(matches) != DDS::RETCODE_OK) {
                ACE_ERROR_RETURN(
                    (LM_ERROR, ACE_TEXT("ERROR: %N:%l: main() -") ACE_TEXT(" get_subscription_matched_status failed!\n")),
                    1);
            }
